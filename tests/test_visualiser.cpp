...
TEST(VisualiserTest, HandlesSingleFace) {
    ...
    cv::Mat faces = (cv::Mat_<float>(1, 15) << 50, 50, 100, 100,
                                             60, 60, 70, 70, 80, 80, 90, 90, 100, 100, 0.99);
    std::string vmodel = "models/MobileFaceNet.onnx";
    std::string mode = "identify";
...
TEST(VisualiserTest, HandlesMultipleFaces) {
    ...
    cv::Mat image = cv::Mat::zeros(224, 224, CV_8UC3);
    // Два "фейковых" лица
    cv::Mat faces = (cv::Mat_<float>(2, 15) <<
      10, 10, 50, 50, 12, 12, 14, 14, 16, 16, 18, 18, 20, 20, 0.95,
      100, 100, 60, 60, 110, 110, 120, 120, 130, 130, 140, 140, 150, 150, 0.98
  );
    std::string vmodel = "models/MobileFaceNet.onnx";
    std::string mode = "identify";
